...
export default function HomePage() {
  ...
  const heroSrc = assets[WEBSITE_ASSET_KEYS.HOME_HERO_BG]?.cloudinaryUrl ?? FALLBACK_HERO;
  const heroAlt = assets[WEBSITE_ASSET_KEYS.HOME_HERO_BG]?.altText ?? "Architectural home";
  const methodologySrc =
    assets[WEBSITE_ASSET_KEYS.HOME_METHODOLOGY_FEATURE]?.cloudinaryUrl ?? FALLBACK_METHODOLOGY;
  const methodologyAlt =
    assets[WEBSITE_ASSET_KEYS.HOME_METHODOLOGY_FEATURE]?.altText ?? "Clinical cleaning methodology";
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!res?.ok) return;
      const json = (await res.json()) as { heroBanner: { headline: string; subtext: string } | null };
      ...
      <section className="bg-clinical-lavender py-16">
        <Container>
          ...
          <p className="mt-4 max-w-2xl text-sm leading-6 text-clinical-charcoal/70">
            Everything we do is built on standards that keep our service dependable, respectful, and
            consistent.
          </p>
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            {[
              ...
              <div key={c.title} className="rounded-xl2 bg-white p-8 shadow-clinicalSm">
                ...
                <div className="mt-2 text-sm leading-6 text-clinical-charcoal/70">{c.body}</div>
              </div>
            ))}
          </div>
        </Container>
            ...
            <div className="mt-10 grid gap-6 sm:grid-cols-2">
              <div className="rounded-xl2 bg-clinical-lavender p-6 shadow-clinicalSm">
                ...
                <div className="mt-2 text-sm leading-6 text-clinical-charcoal/70">
                  A reliable process helps us maintain the same professional standard across every service.
                </div>
              </div>
              <div className="rounded-xl2 bg-clinical-lavender p-6 shadow-clinicalSm">
                <div className="text-sm font-semibold">Careful Handling</div>
                <div className="mt-2 text-sm leading-6 text-clinical-charcoal/70">
                  We treat each property with respect, working carefully around furniture, fittings, and
                  everyday spaces.
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="rounded-xl2 bg-clinical-aqua shadow-clinical">
              <div className="p-6">
                ...
                <div className="mt-6 text-3xl font-semibold text-clinical-navy">Final</div>
                <div className="mt-2 text-sm text-clinical-navy/70">visual check</div>
              </div>
            </div>
            <div className="relative overflow-hidden rounded-xl2 bg-clinical-teal-650 shadow-clinical">
              <div className="absolute inset-0 opacity-30 mix-blend-overlay">
                <img src={methodologySrc} alt={methodologyAlt} className="h-full w-full object-cover" />
              </div>
            </div>
          </div>
        </Container>
