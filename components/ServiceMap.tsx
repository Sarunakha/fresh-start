...
export function ServiceMap({ className = "" }: { className?: string }) {
  ...
  if (!rl || !icon) {
    return (
      <div
        className={[
          "relative min-h-[340px] w-full overflow-hidden rounded-xl2 border border-clinical-aqua/30 bg-white shadow-clinicalSm",
          className
        ]
          .filter(Boolean)
          .join(" ")}
          ...
          <Marker key={z.name} position={z.coords} icon={icon}>
            ...
            >
              <div className="rounded-xl border border-black/10 bg-white px-3 py-2 shadow-clinicalSm">
                <div className="text-xs font-semibold text-[#0A1922]">Service Zone: {z.name}</div>
                <div className="mt-0.5 text-[11px] text-[#0A1922]/70">Clinical Teams Active.</div>
              </div>
            </Tooltip>
          </Marker>
        ))}
